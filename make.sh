...
date=`date +%Y%m%d`
outputname="$romtypename-$outputtype-$sourcever-$date-ErfanGSI-DekuGSIs"
...
sed -i "s/$bdisplay/$displayid2=Built\.with\.ErfanGSI\.Tools\.DekuGSIs/" $systemdir/system/build.prop
